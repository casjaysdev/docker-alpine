...
RUN echo "Initializing the system"; \
  $SHELL_OPTS; \
  if [ -f "/root/docker/setup/init" ];then echo "Running the init script";bash "/root/docker/setup/init";echo "Done running the init script";fi; \
  echo ""
...
RUN echo "Initializing packages before copying files to image"; \
  $SHELL_OPTS; \
  if [ -f "/root/docker/setup/packages" ];then echo "Running the packages script";bash "/root/docker/setup/packages";echo "Done running the packages script";fi; \
  echo ""
...
RUN echo "Running final commands before cleanup"; \
  $SHELL_OPTS; \
  if [ -f "/root/docker/setup/post" ];then echo "Running the post script";bash "/root/docker/setup/post";echo "Done running the post script";fi; \
  echo ""
...
RUN echo "Deleting unneeded files"; \
  ...
  if [ -f "/root/docker/setup/cleanup" ];then echo "Running the cleanup script";bash "/root/docker/setup/cleanup";echo "Done running the cleanup script";fi; \
  echo ""
...
LABEL \
  ...
  org.opencontainers.image.url="https://hub.docker.com/r/casjaysdev/alpine" \
  org.opencontainers.image.url.source="https://hub.docker.com/r/casjaysdev/alpine" \
  org.opencontainers.image.vcs-type="Git" \
  org.opencontainers.image.vcs-ref="${BUILD_VERSION}" \
  org.opencontainers.image.vcs-url="https://github.com/casjaysdev/alpine" \
  org.opencontainers.image.documentation="https://github.com/casjaysdev/alpine" \
